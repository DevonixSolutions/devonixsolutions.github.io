...
class Services {
    ...
    public function __construct() {
        $this->name          = esc_html__( 'Services', 'oxence-toolkit' );
        $this->singular_name = esc_html__( 'Service', 'oxence-toolkit' );
        $this->plural_name   = esc_html__( 'Service', 'oxence-toolkit' );

        $opt        = get_option( 'oxence_options' );
        $this->slug = ! empty( $opt['service_slug'] ) ? strtolower( str_replace( ' ', '', $opt['service_slug'] ) ) : 'services';
    ...
    public function get_single_template( $single_template ) {
        ...
        if ( $post->post_type == $this->type ) {
            if ( file_exists( get_template_directory() . '/single-service.php' ) ) {
                return $single_template;
            }

            $single_template = plugin_dir_path( dirname( __FILE__ ) ) . 'services/templates/single-service.php';
        }
    ...
    public function get_archive_template( $archive_template ) {
        ...
        if ( is_post_type_archive( $this->type ) || ( is_archive() && ! empty( $post->post_type ) && $post->post_type == 'oxence_service' ) ) {
            if ( file_exists( get_template_directory() . '/archive-service.php' ) ) {
                return $archive_template;
            }

            $archive_template = plugin_dir_path( dirname( __FILE__ ) ) . 'services/templates/archive-service.php';
        }
